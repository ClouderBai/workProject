...
export default class Connection {
    ...
    buildConnectionName(fromNode) {
        /*eslint-disable */
        let conns = fromNode.fromConnections,
            name = null
        /*eslint -enable */
        if (conns.length === 0) {
            return null
    ...
    remove() {
        const toConns = this.to.toConnections,
            fromConns = this.from.fromConnections
        const toIndex = toConns.indexOf(this),
    ...
    updatePath() {
        if (this.pathInfo) {
            ...
            const ga = this.g.split(','),
                L = ga.length
            const path = []
            const fromRect = this.from.rect,
                toRect = this.to.rect
            let x1 = fromRect.attr('x'),
                y1 = fromRect.attr('y'),
    ...
    _buildStraightLinePathInfo() {
        ...
        if (this.to) {
            if (pathInfo) {
                const lastLineStart = pathInfo[pathInfo.length - 2]
                path = []
        ...
        if (pathInfo) {
            const pathSegmentLength = pathInfo.length
            const newPathInfo = []
    ...
    _buildFromFigureIntersetion(path, c) {
        ...
        const dot = Raphael.pathIntersection(fromFigurePathInfo, path)
        if (dot.length > 0) {
            const p = {
                x: path[1][1],
                y: path[1][2]
    ...
    _buildToFigureIntersetion(path, c) {
        ...
        const dot = Raphael.pathIntersection(toFigurePathInfo, path)
        if (dot.length > 0) {
            const p = {
                x: path[0][1],
                y: path[0][2]
    ...
    _buildCurveLinePathInfo() {
        ...
        const fromFigurePathInfo = this.from.getPathInfo(true)
        let dis1 = Math.abs((x1 + w1 / 2) - (x2 - w2 / 2)),
            dis2 = Math.abs((y1 + h1 / 2) - (y2 - h2 / 2))
        let line1StartPoint = 'M' + x1 + ' ' + y1
        if (dis1 >= dis2) {
            const firstLine = line1StartPoint + ' L' + x2 + ' ' + y1
            ...
            if (this.to) {
                const lastLine = 'M' + x1 + ' ' + y2 + ' L' + x2 + ' ' + y2
                const toFigurePathInfo = this.to.getPathInfo(true)
            ...
            const firstLine = line1StartPoint + ' L' + x1 + ' ' + y2
            let dot = Raphael.pathIntersection(fromFigurePathInfo, firstLine)
            if (dot.length > 0) {
                x1 = dot[0].x, y1 = dot[0].y
                line1StartPoint = 'M' + dot[0].x + ' ' + dot[0].y
            ...
            if (this.to) {
                const lastLine = 'M' + x2 + ' ' + y1 + ' L' + x2 + ' ' + y2
                const toFigurePathInfo = this.to.getPathInfo(true)
                dot = Raphael.pathIntersection(toFigurePathInfo, lastLine)
                if (dot.length > 0) {
                    x2 = dot[0].x, y2 = dot[0].y
    ...
    _buildText() {
        ...
        if (pathInfo.length === 2) {
            const start = pathInfo[0],
                end = pathInfo[1]
            pos = {
                x: start[1] + (end[1] - start[1]) / 2,
                y: start[2] + (end[2] - start[2]) / 2
            }
        } else {
