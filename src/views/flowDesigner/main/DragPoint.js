...
export default class DragPoint {
    ...
    _init(segmentIndex) {
        ...
        let x, y
        const isCornerPoint = segmentIndex % 2 !== 0
        let targetIndex = Math.round(segmentIndex / 2)
        if (isCornerPoint) {
            const cornerPoint = pathInfo[targetIndex]
            x = cornerPoint[1], y = cornerPoint[2]
        } else {
            let start = pathInfo[targetIndex],
        ...
        this.rect.dblclick(function(e) {
            ...
            if (pi.length === 2) {
                ...
                const ps = pi[1]
                const startDot = _this.connection._buildFromFigureIntersetion({
                    x: ps[1],
                    y: ps[2]
                }, true)
                const startPoint = pi[0]
                startPoint[1] = startDot.x
                startPoint[2] = startDot.y
                const pe = pi[pi.length - 2]
                const endDot = _this.connection._buildToFigureIntersetion({
                    x: pe[1],
                    y: pe[2]
                }, true)
                const endPoint = pi[pi.length - 1]
                endPoint[1] = endDot.x
                endPoint[2] = endDot.y
            }
        ...
        var dragMove = function(dx, dy) {
            ...
            const pi = _this.path.attr('path')
            let L = pi.length,
                dot, p, p1

            const segmentCount = (pathInfo.length - 1) * 2 - 1
            if (segmentIndex === 0 || segmentIndex === 1) {
                p1 = pi[1]
                dot = _this.connection._buildFromFigureIntersetion({
                    x: p1[1],
                    y: p1[2]
                }, true)
                p = pathInfo[0]
        ...
        var dragEnd = function() {
            const newConnectionPathInfo = _this.connection.buildPathInfo(),
